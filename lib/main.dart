...
//
import 'services/crypto_benchmark.dart';
import 'services/timing_logger.dart';
...
class _HealthHomePageState extends State<HealthHomePage>
    ...
    while (mounted) {
      ...
      if (st.status == 'failed') {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Anchor on-chain fallito')),
        );
        break;
      }
  ...
  /// Esegue la suite di benchmark crittografici e logga ogni valore.
  /// I risultati sono visibili con: flutter logs | grep TIMING_CRYPTO
  Future<void> _runCryptoBenchmarks() async {
    ...
    try {
      await CryptoBenchmark.runAll();
      if (!mounted) return;
      setState(() => _benchmarkDone = true);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
            'Benchmark completato! Controlla i log con:\nflutter logs | grep TIMING_',
          ...
          action: SnackBarAction(
            ...
            onPressed: () {
              final csv = TimingLogger.toCsv();
              Clipboard.setData(ClipboardData(text: csv));
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text('CSV copiato negli appunti'),
      ...
      if (!mounted) return;
      setState(() => _benchmarkDone = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Errore benchmark: $e'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    } finally {
  ...
  Future<void> _uploadToIpfs() async {
    ...
    try {
      ...
      if (res.ok && res.cid != null) {
        setState(() {
          _ipfsCid = res.cid;
          _uploadedOk = true; // segnala successo
        });
        ...
        if (_manifestBase != null) {
          ...
          if (saved.ok && mounted) {
            ...
            try {
              ...
              debugPrint('Anchor relayed fallito: $e');
            }
            ...
            debugPrint('Salvataggio manifest fallito');
          }
        }
      ...
      if (mounted) setState(() => _uploadingIpfs = false); // 
    }
  }
  ...
  Future<void> _loadHealthData() async {
    ...
    try {
      ...
      // applica overlap per catturare backfill
      ...
      // filtra duplicati: tieni solo punti successivi all'ultima sync
      ...
      // costruisci payload compatto per cifratura
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              // ─── Benchmark crittografico (per tesi) ───────────────────
              Padding(
                padding: const EdgeInsets.fromLTRB(
                    AppTheme.paddingScreen,
                    AppTheme.gapSmall,
                ...
                child: SizedBox(
                  ...
                  height: 52,
                  child: OutlinedButton.icon(
                    onPressed: _benchmarkRunning ? null : _runCryptoBenchmarks,
                    ...
                    style: OutlinedButton.styleFrom(
                      foregroundColor: _benchmarkDone
                          ? Colors.green[700]
                          : AppTheme.primaryMedium,
                      side: BorderSide(
                        color: _benchmarkDone
                            ? Colors.green[400]!
...
// helper per ricavare la DEK dall'owner wrap (AES-GCM sotto KEK_device)
